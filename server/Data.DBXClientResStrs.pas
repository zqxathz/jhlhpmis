...
resourcestring
  SAreYouSurePolicy =
    'Are you sure you want to delete the access policy from line %d ?';
  ...
  SRootContainerDeleteFailure =
    'Root Container deletion failed with code: %d. A possible solution is to try again after a refresh.';
  SRootContainerCreateFailure =
    'Root Container creation failed with code: %d. A possible solution is to try again after a refresh.';
  ...
  SLongTimeOperation =
    'Please allow 10 minutes per MB for the operation to complete';
  ...
  SDeleteContainerMsg =
    'You are about to delete the %s container and all its content. Proceed with the deletion?';
  SDeleteBlobMsg =
    'You are about to delete the blob %s from the %s container. Proceed with the deletion?';
  ...
  SMetadataBlobFailure =
    'Blob''s %s/%s metadata access failed. Error code is: %d.';
  SMetadataBlobCaption = 'Edit Blob %s/%s Metadata';
  SMetadataContainerFailure =
    'Container''s %s metadata access failed. Error code is: %d.';
  SMetadataContainerCaption = 'Edit Container %s Metadata';
  SPropertiesFailure =
    'Accessing the properties of %s/%s blob failed. Error code is: %d.';
  ...
  SDestContainerNotFound =
    'The destination container %s cannot be found. You can create it or refresh the storage content';
  ...
  SCommitBlockListFailure =
    'Committing the block list of %s/%s failed. Error code is: %d.';
  SBlockListIsEmpty =
    'The block list is empty for blob %s/%s. Please use %s to add blocks into it and then commit them.';
  SLeaseItem = 'Lease...';
  ...
  SLeaseBreakNotice =
    'Lease on %s/%s is broken, a new lease can be acquired in %d seconds';
  ...
  SSharedKeyError =
    'The shared key cannot be created. Please check that you have the required OpenSSL libraries.';
  SMissingServerSettings =
    'Cannot connect %s to server, SQLConnection and ServerClassName must both be set.';
  sUnknownCommandType = 'Unknown command type';
  SNumberFormatError = 'The parameter has an invalid number format';
  SInvalidTunnel =
    'The callback tunnel is in an invalid state and should be terminated.';
  SProtocolNotSupported =
    'Specified protocol in request %s is not supported. Current supported protocols are: %s, json, tunnel';
  SProtocolRestEmpty = 'none specified (uses rest)';
  SCommandNotSupported =
    'Command %s in request is not supported. Accepted commands are: GET, POST, PUT and DELETE';
  ...
  SInvalidRequestFormat =
    'Invalid request format. /className/methodName and eventual parameters expected.';
  STooManyParameters =
    'Server method %0:s has only %1:d parameters. Too many arguments actual arguments: %2:d.';
  SNotInputParameter =
    'Server''s method %0:s %1:d parameter in not an input parameter. Either call with less parameters or change the method definition.';
  ...
  STooManyInputParams =
    'Server method %0:s consumed only %1:d input parameters out of %2:d. Either call with more parameters or change method definition.';
  SNoJSONCmdContent =
    'Request type %0:s has an empty content. A JSON command object is expected';
  SPOSTExpected =
    'JSON protocol expects a POST request instead of %0:s. Submit a POST request with a JSON command to receive proper response';
  SJSONArrayExpected =
    'JSON protocol expects a JSON array instead a %0:s type. Submit a POST request with a JSON command to receive proper response';
  SNoSessionFound = 'Session expired';
  SNoSessionInRequest =
    'HTTP request is missing the session parameter. Add %s parameter with the session id.';
  SNoCountParam =
    'HTTP request misses the count parameter. Add %s parameter holding the byte count.';
  ...
  SBadParamName =
    'Invalid parameter naming. Expected [dcname.]field[numeric range] instead of %s';
  SCannotConvertParam =
    'The output parameter (%d) cannot be converted by the selected converter (%s)';
  SJSONValueNotObject = 'Value must be an instance of TJSONObject';
  SCommandNotImplemented = 'Command %s not implemented';
  SNoRegisteredCallbackForId =
    'Invocation failed as there is currently no registered callback for the given id (%s)';
  SRegisteredCallbacksExist =
    'Channel is terminated while %d are still registered with it';
  SAuthenticationFailed = '服务器拒绝当前用户登录';
  // 'Authentication manager rejected user credentials. This may due to invalid combination of DS user name and password';
  SSessionExpiredMsg =
    'The specified session has expired due to inactivity or was an invalid Session ID';
  SServerClassExists =
    '%s class has already been added to the server class list';
  ...
  SNoCacheParameter =
    'There is no parameter for command %d, parameter index %d.';
  SConError =
  ...
  SProtocolErrorSize =
    'Response size (%d bytes) is greater than expected (%d bytes).';
  SConnectionLost = 'Connection lost';
  ParameterNotSet = 'Parameter not set for column number %d';
  SNoRegisteredLayer =
    'Protocol %0:s can be used after an adequate instance of %1:s is registered with %2:s.';
  ...
  SCallbackBadInput =
    'Callback input argument cannot be parsed properly. This is due to a parser error or the server side-serialization';
  SBadCallbackParameter =
    'A callback parameter cannot be found at the provided index';
  ...
  SCannotLoadMetaDataForJds7 =
    'MetaData cannot be provided from a Blackfish SQL server older than version 8.0';
  ...
  SNoSpaceAvailable =
    'Operation %s cannot complete since it expects a container for at least %s bytes instead of %s';
  ...
  SFilterNotRegistered =
    'Communication filter %s is not registered. Filter class needs to be registered in order to communicate with the server.';
  SInvalidFilterParameter =
    'Filter %s rejected setup parameter %s given value %s. At this point the server communication is not possible due to this incompatibility.';
  ...
  sCannotChangeIPImplID =
    'IPImplementationID cannot be change while transport is active';
