...
unit plugin_admin_servermethods;
...
type
  ...
  TServerAdminMethodsClient = class(TDSAdminClient)
  private
    FEchoStringCommand: TDBXCommand;
    FGetDataCommand: TDBXCommand;
    FPostDataCommand: TDBXCommand;
  public
    constructor Create(ADBXConnection: TDBXConnection); overload;
    constructor Create(ADBXConnection: TDBXConnection; AInstanceOwner: Boolean); overload;
    destructor Destroy; override;
...
function TServerAdminMethodsClient.EchoString(Value: string): string;
begin
  if FEchoStringCommand = nil then
  begin
    FEchoStringCommand := FDBXConnection.CreateCommand;
    FEchoStringCommand.CommandType := TDBXCommandTypes.DSServerMethod;
    FEchoStringCommand.Text := 'TServerAdminMethods.EchoString';
    FEchoStringCommand.Prepare;
  end;
  FEchoStringCommand.Parameters[0].Value.SetWideString(Value);
  FEchoStringCommand.ExecuteUpdate;
...
function TServerAdminMethodsClient.GetData: TStream;
begin
  ...
  begin
    ...
    FGetDataCommand.Text := 'TServerAdminMethods.GetData';
    FGetDataCommand.Prepare;
  end;
  FGetDataCommand.ExecuteUpdate;
  Result := FGetDataCommand.Parameters[0].Value.GetStream(FInstanceOwner);
end;

function TServerAdminMethodsClient.PostData(stream: TStream): string;
begin
  if FPostDataCommand = nil then
  begin
    FPostDataCommand := FDBXConnection.CreateCommand;
    FPostDataCommand.CommandType := TDBXCommandTypes.DSServerMethod;
    FPostDataCommand.Text := 'TServerAdminMethods.PostData';
    FPostDataCommand.Prepare;
  end;
  FPostDataCommand.Parameters[0].Value.SetStream(stream, FInstanceOwner);
  FPostDataCommand.ExecuteUpdate;
  Result := FPostDataCommand.Parameters[1].Value.GetWideString;
end;


constructor TServerAdminMethodsClient.Create(ADBXConnection: TDBXConnection);
begin
  inherited Create(ADBXConnection);
end;


constructor TServerAdminMethodsClient.Create(ADBXConnection: TDBXConnection; AInstanceOwner: Boolean);
begin
  inherited Create(ADBXConnection, AInstanceOwner);
end;


destructor TServerAdminMethodsClient.Destroy;
begin
  FEchoStringCommand.DisposeOf;
  FGetDataCommand.DisposeOf;
  FPostDataCommand.DisposeOf;
  inherited;
end;
